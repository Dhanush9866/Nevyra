    ...
    <>
      ...
      <div className="min-h-screen bg-background py-12 px-4 md:px-12">
        <div className="max-w-6xl mx-auto">
          <h1 className="text-3xl font-bold mb-6 bg-gradient-primary bg-clip-text text-transparent">Products</h1>
                  ...
                  <tbody className="bg-card divide-y divide-border">
                    {dummyProducts.map((product) => (
                      <tr key={product.id} className="hover:bg-muted/50 transition-smooth">
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-foreground">{product.id}</td>
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-foreground">{product.price}</td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-foreground">{product.stock}</td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm">
                          <span className={`px-3 py-1 rounded-full text-xs font-semibold ${statusColor(product.status)}`}>{product.status}</span>
                        </td>
                      </tr>
                    ))}
                  </tbody>
